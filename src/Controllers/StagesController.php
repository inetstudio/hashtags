...
use InetStudio\Hashtags\Requests\SaveStageRequest;
use InetStudio\Hashtags\Transformers\StageTransformer;

/**
 * Контроллер для управления этапами.
 *
...
class StagesController extends Controller
{
    ...
    public function index(Datatables $dataTable)
    {
        ...
        $table->ajax([
            'url' => route('back.hashtags.stages.data'),
            'type' => 'POST',
    ...
    {
        return view('admin.module.hashtags::pages.stages.form', [
            'item' => new StageModel(),
        ]);
    }
     ...
     */
    public function store(SaveStageRequest $request)
    {
    ...
    {
        if (! is_null($id) && $id > 0) {
            $item = StageModel::where('id', '=', $id)->first();
        } else {
            abort(404);
        }
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(SaveStageRequest $request, $id = null)
    {
    ...
    private function save($request, $id = null)
    {
        ...
        return redirect()->to(route('back.hashtags.stages.edit', $item->fresh()->id));
    }

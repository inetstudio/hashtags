...
namespace InetStudio\Hashtags\Services\Front\Posts;
...
use InetStudio\Hashtags\Contracts\Services\Front\Posts\ContestPostsServiceContract;
...
class ContestPostsService implements ContestPostsServiceContract
{
     ...
     * @return array
     */
    public function getGallery(Request $request, string $social = ''): array
    {
     ...
     * @return array
     */
    public function getDaysWinners(string $prizeAlias = ''): array
    {
        $mainStatuses = StatusModel::whereHas('classifiers', function ($classifiersQuery) {
            $classifiersQuery->where('classifiers.alias', 'main');
        })->pluck('id')->toArray();
     ...
     * @return array
     */
    public function getStagesWinners(string $stageAlias = '', string $prizeAlias = ''): array
    {
        $mainStatuses = StatusModel::whereHas('classifiers', function ($classifiersQuery) {
            $classifiersQuery->where('classifiers.alias', 'main');
        })->pluck('id')->toArray();
     ...
     * @return array
     */
    protected function getPostData(PostModel $item): array
    {
     ...
     * @return string
     */
    protected function get_correct_str(int $num, array $variant): string
    {
        $val = $num % 100;
     ...
     * @return string
     */
    protected function getPointsWord(int $num): string
    {
        $num = (! $num) ? 0 : $num;
